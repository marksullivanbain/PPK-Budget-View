...
export function ProgramByAccount({ data }: ProgramByAccountProps) {
  ...
  return (
    <Card className="p-5 flex flex-col border-card-border h-full" data-testid="card-program-by-account">
      <div className="flex flex-col gap-1 mb-4">
        ...
        <p className="text-sm text-muted-foreground">Breakdown by summary account</p>
      </div>
      
      <ScrollArea className="flex-1 -mx-1 px-1">
        <div className="flex flex-col gap-3 pr-2">
          {data.length === 0 ? (
            <p className="text-sm text-muted-foreground">No program data above threshold</p>
          ) : (
            data.map((item) => {
