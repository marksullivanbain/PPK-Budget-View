...
export async function registerRoutes(
  ...
  app.get("/api/admin-comparison", isAuthenticated, async (req, res) => {
    try {
      const userEmail = getUserEmail(req);
      const allowedPractices = getPracticesForEmail(userEmail || '');
      ...
      const ytdDec2025 = await storage.getAdminSummary('ytd', 12, 2025);
      const ytdSep2025 = await storage.getAdminSummary('ytd', 9, 2025);
      const janFeb2026 = await storage.getAdminSummary('ytd', 2, 2026);

      const subtractPeriod = (full: typeof ytdDec2025, partial: typeof ytdSep2025) => {
        const practices = full.practices.map(fp => {
          const pp = partial.practices.find(p => p.practice === fp.practice);
          const actuals = {
            compensation: fp.actuals.compensation - (pp?.actuals.compensation || 0),
            programs: fp.actuals.programs - (pp?.actuals.programs || 0),
            databases: fp.actuals.databases - (pp?.actuals.databases || 0),
            bcn: fp.actuals.bcn - (pp?.actuals.bcn || 0),
            total: fp.actuals.total - (pp?.actuals.total || 0),
          };
          const budget = {
            compensation: fp.budget.compensation - (pp?.budget.compensation || 0),
            programs: fp.budget.programs - (pp?.budget.programs || 0),
            databases: fp.budget.databases - (pp?.budget.databases || 0),
            bcn: fp.budget.bcn - (pp?.budget.bcn || 0),
            total: fp.budget.total - (pp?.budget.total || 0),
          };
          const variance = {
            compensation: budget.compensation - actuals.compensation,
            programs: budget.programs - actuals.programs,
            databases: budget.databases - actuals.databases,
            bcn: budget.bcn - actuals.bcn,
            total: budget.total - actuals.total,
            percentVariance: budget.total !== 0 ? Math.round(((budget.total - actuals.total) / budget.total) * 100) : 0,
          };
          return { ...fp, actuals, budget, variance };
        });
        const totals = {
        ...
        practices.forEach(p => {
          (['compensation', 'programs', 'databases', 'bcn', 'total'] as const).forEach(k => {
            totals.actuals[k] += p.actuals[k];
            totals.budget[k] += p.budget[k];
            totals.variance[k] += p.variance[k];
          });
        });
        totals.variance.percentVariance = totals.budget.total !== 0 
          ? Math.round((totals.variance.total / totals.budget.total) * 100) : 0;
        return { practices, totals };
      };

      const q4Data = subtractPeriod(ytdDec2025, ytdSep2025);
      res.json({
        q4: { ...q4Data, label: 'Q4 2025 (Oct-Dec)' },
        janFeb: { 
          practices: janFeb2026.practices, 
          totals: janFeb2026.totals, 
          label: 'Jan-Feb 2026' 
        },
      });
    } catch (error) {
      res.status(500).json({ error: "Failed to fetch admin comparison" });
    }
  });
